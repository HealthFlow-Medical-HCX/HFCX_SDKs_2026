...
that the .NET SDK ships with. Mirror of the Java and Python SDKs'
`fhir-ig/`.

| File                  | Purpose                                                  |
|-----------------------|----------------------------------------------------------|
| `egyptian-ig.tgz`     | The IG package (NPM-style FHIR package). **Synced from `hfcx-platform` releases.** Not committed until Sprint D6. |
| `PLATFORM_VERSION`    | Plain-text file recording which platform release the bundled `egyptian-ig.tgz` came from. Read at runtime by `HfcxSdk.BundledIgVersion`. |
...
5. (Sprint D6+) Embeds the IG package as a NuGet content file so the
   wheel ships with it.
