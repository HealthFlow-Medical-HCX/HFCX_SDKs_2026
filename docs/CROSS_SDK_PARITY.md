...
| JavaScript | 🚧 Sprint S2   | 0.1.0-alpha.0    | gated on npm Trusted Publisher |
...
| 24 | Layer toggles (4 layers)         | ✅ `Layer` enum + `Builder#enable`  | ✅ `Layer` Enum + `enabled_layers=`     | ✅ `Layer` enum + `enabledLayers:`    | `Layer` literal type                |
